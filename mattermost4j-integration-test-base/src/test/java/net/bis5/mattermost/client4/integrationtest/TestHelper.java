...
import static net.bis5.mattermost.client4.integrationtest.Assertions.checkNoError;
...
public class TestHelper {
  ...
  private Path ensureTempDir() {
    if (tmpDir == null) {
      try {
        tmpDir = Files.createTempDirectory(TestHelper.class.getSimpleName(), PosixFilePermissions.asFileAttribute(PosixFilePermissions.fromString("rwxrwxrwx")));
      } catch (IOException e) {
  ...
  public Path getResourcePath(String name) throws URISyntaxException, IOException {
    Path extracted = ensureTempDir().resolve(name.replace("/", ""));
    Files.createFile(extracted);
    Files.copy(getClass().getResourceAsStream(name), extracted, StandardCopyOption.REPLACE_EXISTING);
    return extracted;
  }
